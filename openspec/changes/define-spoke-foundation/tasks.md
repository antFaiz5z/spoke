...
- [ ] 设计练习页组件树
- 当前主舞台已可用，但仍主要集中在单个 `PracticeStage` 客户端组件中，尚未拆分为稳定组件树。
...
- [ ] 避免使用影响固定吸附布局稳定性的 magic numbers
- 当前舞台页已改为通过真实测量顶部栏与底部播放器高度来计算内容避让；后续页面也需要遵守同一原则。
- [ ] 固化固定吸附 UI 的共享布局 token，避免头部、正文、草稿层、播放器各自维护一套间距常量
- 当前舞台页已抽出共享布局 token 与视觉 token，但尚未提升为跨页面约束。
- [ ] 保持页面主组件以编排职责为主，拆分控制器、布局层、视觉层与纯函数状态派生
- 当前舞台页已开始拆分 `controller`、`layout`、`visuals` 与子组件；其他页面尚未按同一粒度整理。
- [ ] 在存在独立滚动容器时，统一监听真实滚动源，而非默认绑定 `window`
- 当前舞台页已修正此问题，并补充了对应测试；后续页面需沿用同一规则。
- [ ] 将移动端适配与 safe area 处理视为默认约束，而不是上线前补丁
